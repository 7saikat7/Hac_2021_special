...
const deleteSession = async (token, ip) => {
  try {
    console.log("Parameter that came for deleting: ", token, "IP: ", ip);
    let session = await activeUser.deleteOne({ auth_token: token });
    console.log("NUmber of Session deleted: ", session.deletedCount);
